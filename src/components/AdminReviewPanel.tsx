...
/**
 * Convert a classification pathname to the storage key used everywhere else:
 *   /classification/a/leadership  → classificationA_leadership
 *   /classification/b/social-media → classificationB_social_media
 */
function pathnameToStorageKey(pathname: string): string {
  const match = pathname.match(/\/classification\/([ab])\/(.+)/i);
  if (!match) return '';
  const type = match[1].toUpperCase();
  const axis = match[2].replace(/-/g, '_');
  return `classification${type}_${axis}`;
}
...
export default function AdminReviewPanel() {
  ...
  // Use the same key format as AcademyDashboard and AdminReviewDossier
  const axisKey = pathnameToStorageKey(location.pathname);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        // Read from the same subcollection that AdminReviewDossier writes to
        const reviewDoc = await getDoc(
          doc(db, "users", adminViewUid, "adminReviews", axisKey),
        );
        if (reviewDoc.exists()) {
          const data = reviewDoc.data();
          setNotes(data.note || data.notes || "");
          setStatus(data.status === "مقبول" ? "approved" : data.status === "مرفوض" ? "rejected" : data.status || "pending");
        }
      } catch (e) {
    ...
    loadData();
  }, [adminViewUid, axisKey]);

  const handleSave = async () => {
    if (!adminViewUid || !axisKey) return;
    setIsSaving(true);
    try {
      // Write to the subcollection — consistent with AdminReviewDossier and AcademyDashboard
      await setDoc(
        doc(db, "users", adminViewUid, "adminReviews", axisKey),
        {
          status,
      ...
      <div className="max-w-[1240px] mx-auto px-4 py-4 flex flex-col md:flex-row gap-4 items-center">
        ...
        <div className="flex-1 w-full">
          <input
            ...
            placeholder="أضف ملاحظاتك أو أسباب الرفض..."
